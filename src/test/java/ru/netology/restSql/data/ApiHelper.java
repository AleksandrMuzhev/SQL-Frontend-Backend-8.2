...
public class ApiHelper {
    ...
    public static void makeQueryToLogin(DataHelper.AuthInfo authInfo, Integer statusCode) {
        given() //запрос на выполнение логина
                .spec(requestSpec)
                .body(authInfo)
                .when()
                .post("/api/auth")
    ...
    @Value //Дата класс для получения токена
    public static class APITokenInfo {
        String token;
    }

    @Value //Дата класс для описания ответа баланса карт
    ...
    @Value //Дата класс для описания перевода откуда, куда, сумма
